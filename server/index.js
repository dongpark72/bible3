...
app.use(cors({
    origin: ['https://bible.goal-runner.com', 'http://localhost:8005'],
...
app.post('/api/login', (req, res) => {
        ...
        if (db.users[username].password === password) {
            return res.json({
                ...
                user: {
                    ...
                    wordbook: db.users[username].wordbook || [],
                    highlights: db.users[username].highlights || {},
    ...
    writeDB(db);
    res.json({ success: true, user: { username, group: '교구', wordbook: [], highlights: {}, history: [] } });
});

// Sync User Data (Wordbook, Highlights, History)
app.post('/api/sync-data', (req, res) => {
    const { username, wordbook, highlights, history } = req.body;
    const db = readDB();

    if (db.users[username]) {
        if (wordbook) db.users[username].wordbook = wordbook;
        if (highlights) db.users[username].highlights = highlights;
