// Final fix attempt: 2026-02-16 23:00
const BOOK_NAMES = {
...
class BibleApp {
    ...
    async init() {
        try {
            // Load book list metadata instead of the whole Bible
            const response = await fetch('data/ko/metadata.json');
            const meta = await response.json();

            // Reconstruct minimal bibleData from metadata
            this.bibleData = meta.map(m => ({
                book: m.book,
                abbrev: m.abbrev,
                chapters: new Array(m.chapterCount).fill(null) // Placeholders
            }));
    ...
    async loadAuxBible(version) {
        // Now handled on-demand in loadChapter
        console.log(`Version ${version} will be loaded on-demand.`);
    }
    ...
    async loadChapter(bookIdx, chapIdx, shouldScroll = true) {
        ...
        if (!book.chapters[chapIdx]) {
            try {
                this.elements.verseDisplay.innerHTML = '<div class="loading-state">성경 데이터를 불러오는 중...</div>';
                const res = await fetch(`${API_BASE}/bible/ko/${book.abbrev}.json`);
                const fullBookData = await res.json();
                book.chapters = fullBookData.chapters;
            } catch (e) {
                console.error('Failed to load book from R2:', e);
                this.elements.verseDisplay.innerHTML = '<div class="error">데이터를 불러오지 못했습니다. R2 설정을 확인해 주세요.</div>';
                return;
            }
        }
        ...
        if (auxVer) {
            // Fetch auxiliary book data if not already loaded
            if (!this[`bibleData_${auxVer}`]) {
                this[`bibleData_${auxVer}`] = [];
            }
            let auxBook = this[`bibleData_${auxVer}`][bookIdx];

            if (!auxBook || !auxBook.chapters[chapIdx]) {
                try {
                    const res = await fetch(`${API_BASE}/bible/en_${auxVer}/${book.abbrev}.json`);
                    if (res.ok) {
                        const fullAuxBook = await res.json();
                        this[`bibleData_${auxVer}`][bookIdx] = fullAuxBook;
                        auxBook = fullAuxBook;
                    }
                } catch (e) {
                    console.warn(`Failed to load auxiliary book ${auxVer}:`, e);
                }
            }
    ...
    setupInstallPrompt() {
        if (this.installPromptSetup) return;
        this.installPromptSetup = true;
        ...
        window.addEventListener('beforeinstallprompt', (e) => {
            ...
            // Also show a toast or highlight the button if needed
